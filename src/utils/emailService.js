...
exports.sendOTP = async (to, code) => {
    ...
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px; background: #f9fafb; border-radius: 12px;">
        <h2 style="color: #1f2937; margin: 0 0 8px;">onlineocrfree — Verification Code</h2>
    ...
    await transporter.sendMail({
        from: `"onlineocrfree" <${process.env.SMTP_USER}>`,
        to,
        subject: `${code} — Your onlineocrfree Verification Code`,
        html,
    });
};
...
exports.sendResults = async (to, downloadUrl, format = 'zip') => {
    ...
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px; background: #f9fafb; border-radius: 12px;">
        ...
        <p style="color: #9ca3af; font-size: 12px; margin: 0;">This link expires in 7 days. Sent from onlineocrfree.</p>
    </div>`;

    await transporter.sendMail({
        from: `"onlineocrfree" <${process.env.SMTP_USER}>`,
        to,
        subject: 'Your OCR Results are Ready — onlineocrfree',
        html,
    });
};
