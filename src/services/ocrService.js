    ...
    if (mimetype === 'application/pdf') {
        ...
        for (let i = 0; i < imageFiles.length; i++) {
            ...
            if (useGoogleVision) {
                ...
                text = await geminiEngine.extractText(image, 'image/png', lang, geminiApiKey, openRouterOutputFormat, customPrompt, geminiCustomModel);
        ...
        if (useGoogleVision) {
            ...
            console.log('🔍 Running OCR with Gemini Flash...');
            textResult = await geminiEngine.extractText(filePath, mimetype, lang, geminiApiKey, openRouterOutputFormat, customPrompt, geminiCustomModel);
        } else if (useOpenRouter) {
            console.log(`🔍 Running OCR with OpenRouter (${engine === 'openrouter-custom' ? openRouterCustomModel : engine})...`);
